...
function shouldTakeUmbrella() {
    if (isRaining === 'yes' && hasJacket === 'no' && isSummer === 'no') {
        return 'Please take an umbrella'
    } return 'No real need for an umbrella today!'
...
function canRideRollerCoaster() {
    ...
    return "Can ride!" + ' ' + true;
        
    } 
    return "Sorry kiddo, no can do." +  ' ' + false;
    }
...
function canRollOnTheCoaster(userAge, userHeight) {
    let conditions = userAge >= 18 && userHeight > 175;
    switch (conditions) {
        case true: {
            return "Can ride!" + ' ' + true; }
        case false: {
...
console.log(canRollOnTheCoaster(18, 200));
console.log(canRollOnTheCoaster(11, 200));
console.log(canRollOnTheCoaster(111, 150));
console.log(canRollOnTheCoaster(11, 151));
...
function slugify(sentence) {
    let sentenceWithoutSpaces = sentence.replace(/ /g,'' );
    let unifiedLetters = sentenceWithoutSpaces.toLowerCase();
    let choppedSentence = unifiedLetters.split("");
    let slug = choppedSentence.join('-');
    return slug
}


console.log(slugify('Mam na imię Gosia.'));
console.log(slugify('Lubię układać puzzle dla dorosłych.'));
// Zadanie 18.

function toUpperCase(sentence) {
    let wordsChopped = sentence.split(" ");
    let firstWord = wordsChopped[0].split('');
    let firstLetterCorrected = firstWord[0].toUpperCase();
    let joinedFirstWord = firstLetterCorrected + firstWord.join('');
    let correctedFirstWord = joinedFirstWord.replace(joinedFirstWord[1], '');
    let sentenceWithoutFirstWord = wordsChopped.slice(1);
    let correctedSentence = correctedFirstWord + ' ' + sentenceWithoutFirstWord.join(' ');
    return correctedSentence;
}
console.log(toUpperCase('psy to lepsze zwierzęta domowe niż koty.'));
console.log(toUpperCase('trefl to świetna jakość w niskiej cenie.'));
